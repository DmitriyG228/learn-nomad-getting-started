...
# Configuration
REDIS_CONTAINER="${REDIS_CONTAINER:-$(docker ps --format '{{.Names}}' | grep '^redis-' | head -1)}"
REFRESH_INTERVAL="${REFRESH_INTERVAL:-2}"
SHOW_HEARTBEATS="${SHOW_HEARTBEATS:-true}"

# Fallback to development container if no Nomad Redis found
if [ -z "$REDIS_CONTAINER" ] || [ "$REDIS_CONTAINER" = "$(docker ps --format '{{.Names}}' | grep '^redis-' | head -1)" ] && [ -z "$(docker ps --format '{{.Names}}' | grep '^redis-')" ]; then
